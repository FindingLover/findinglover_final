...
import math
import os
from settings import  BASE3, WAVE,PATH31,PATH32,PATH33
...
VIRUS3_IMAGE = pygame.image.load(os.path.join("images", "virus4.png"))
class Enemy:
    def __init__(self):
        self.path = PATH31
       # self.path_pos = 0
    ...
    @classmethod
    def virus3(cls):  # 第三關再出現
        virus3 = cls()
        virus3.image = pygame.transform.scale(VIRUS3_IMAGE, (68, 78))
        return virus3
...
class EnemyGroup:
    ...
    def add(self, num):
        ...
        #for i in range(0, 5):
        if  not self.is_empty() :#and self.is_empty2():
            p = random.randint(0, 2)
            for i in range(num):
